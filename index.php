...
<script>
  ...
  const pair1 = { pair: "BTCUSDT" };
  const pair2 = { pair: "ETHUSDT" };
  const pair3 = { pair: "ETHBTC" };

  const url1 = `${endpoint}${pair1.pair}${pair2.pair}?${querystring}`;
  const url2 = `${endpoint}${pair2.pair}${pair3.pair}?${querystring}`;
  const url3 = `${endpoint}${pair3.pair}${pair1.pair}?${querystring}`;

  Promise.all([
    fetch(url1, { headers }).then((response) => response.json()),
    fetch(url2, { headers }).then((response) => response.json()),
    fetch(url3, { headers }).then((response) => response.json()),
  ]).then((data) => {
    // Use the data returned from the API calls here
    console.log(data);
  }).catch((error) => {
    // Handle errors here
    console.error(error);
